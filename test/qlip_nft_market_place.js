 ...
 */
contract('QlipMarketPlace', async function ([deployer, addressOne, addressTwo]) {
  let name, symbol, qlipContract

  before(async () => {
    name = 'Qlip NFT Marketplace'
    symbol = 'QLIP NFT'
    qlipContract = await QlipMarketPlace.new(name, symbol)
  })
  ...
  it('should assert token Mint event', async function () {
    const _e = await qlipContract.mintWithIndex(deployer, ['https://token-url.com'])
